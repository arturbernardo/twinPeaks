  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">{team.description} · {members.length} people</p>
      </div>

      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Team culture profile</CardTitle>
          </CardHeader>
              ...
              <div key={t.tagId}>
                <div className="flex items-baseline justify-between text-sm">
                  ...
                  <span className="font-mono text-xs text-muted-foreground">
                    ...
                    {t.top && <> · driven by {t.top.employee.name.split(" ")[0]}</>}
                  </span>
                </div>
        ...
        <div className="space-y-6">
          {outliers.length > 0 && (
            <Card className="border-violet-200 dark:border-violet-900">
              <CardHeader>
                <CardTitle className="text-base">Team outlier</CardTitle>
              </CardHeader>
              <CardContent>
                ...
                <p className="mt-2 text-sm text-muted-foreground">Stands out from the team average in:</p>
          ...
          {gaps.length > 0 && (
            <Card className="border-amber-200 dark:border-amber-900">
              <CardHeader>
                <CardTitle className="text-base">Gaps vs. the “startup” archetype</CardTitle>
                <p className="text-xs text-muted-foreground">Tags where no member reaches the archetype&apos;s target.</p>
              </CardHeader>
              <CardContent className="space-y-2">
                {gaps.map((g) => (
                  <div key={g.tagId} className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-mono text-xs text-muted-foreground">
                      max {g.teamMax.toFixed(2)} / target {g.target.toFixed(2)}
      ...
      <div className="space-y-5">
        ...
        {[
          { id: null as string | null, name: "Department leadership" },
      ...
      {former.length > 0 && (
        <div>
          <h2 className="mb-2 text-sm font-medium text-muted-foreground">Former members</h2>
