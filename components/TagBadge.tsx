...
const THEORY_STYLES: Record<string, string> = {
  ...
  Practice: "bg-violet-50 text-violet-700 border-violet-200 dark:bg-violet-950 dark:text-violet-300 dark:border-violet-900",
};
...
export function SourceBadge({ source }: { source: "self" | "peer" | "manager" }) {
  const map = {
    self: { label: "self-report", cls: "bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950 dark:text-amber-300" },
    peer: { label: "peer", cls: "bg-sky-50 text-sky-700 border-sky-200 dark:bg-sky-950 dark:text-sky-300" },
    manager: { label: "manager", cls: "bg-violet-50 text-violet-700 border-violet-200 dark:bg-violet-950 dark:text-violet-300" },
  } as const;
  const m = map[source];
